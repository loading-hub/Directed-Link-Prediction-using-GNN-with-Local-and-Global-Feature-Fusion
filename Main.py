...
def test(model: Model, dataset, x, edge_index, edge_weight, y, final=False):
    ...
    np.save('c://embeding.npy', torch.cat(((torch.from_numpy(embeding)),(node_tag)),axis=1))
...
if __name__ == '__main__':
    ...
    A = net.copy()  
    A[test_pos[0], test_pos[1]] = 0  
    A[test_pos[1], test_pos[0]] = 0  
    A.eliminate_zeros()
    ...
    parser1.add_argument('--dataset', type=str, default=dst)  
    ...
    train_graphs, test_graphs, max_n_label = links2subgraphs(A, train_pos, train_neg, test_pos, test_neg, args.hop, args.max_nodes_per_hop,fea1)
    ...
    cmd_args.feat_dim = (max_n_label + 1 + dim)*2   
    cmd_args.attr_dim = 0
